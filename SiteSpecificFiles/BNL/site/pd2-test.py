...
#    tssub.synchCommand(10,"setTickMillis",60000);
...
#    biassub.asynchCommand("accumBuffer",3000,nplc,True);
    print "time before bias device setup = %f" % time.time()
    biassub.synchCommand(100,"accumBuffer",2048,nplc,False);
    print "time after bias device setup = %f" % time.time()
    time.sleep(0.2);
    print "time before pd device setup = %f" % time.time()
    pdsub.synchCommand(100,"accumBuffer",2048,nplc,False);
    print "time after pd device setup = %f" % time.time()
